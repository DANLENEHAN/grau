...
class TestUtils:
    ...
    def test_get_secret_key(self):
        """
        Test that the function returns the APP_SECRET environment variable.
        """
        # given
    ...
    def test_get_secret_key_none(self):
        """
        Test that the function returns None if the APP_SECRET environment variable is not set.
        """
        # given
    ...
    def test_encrypt_str(self, mock_fernet: MagicMock):
        """
        Test that the function encrypts a string using a secret key.
        """
        # given
        ...
        mock_fernet.return_value.encrypt.assert_called_once_with(secret_str.encode())
        mock_fernet.return_value.encrypt.return_value.decode.assert_called_once()
    ...
    def test_decrypt_str(self, mock_fernet: MagicMock):
        ...
        # then
        mock_fernet.assert_called_once_with(self.app_secret)
        mock_fernet.return_value.decrypt.assert_called_once_with(secret_str.encode())
        mock_fernet.return_value.decrypt.return_value.decode.assert_called_once()
